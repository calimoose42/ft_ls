...
t_files		*dir_recursive(t_files *sub_dirs, t_opt option) 	
{
	...
	option.R_par = (option.R_par == NULL) ? sub_dirs : option.R_par;	//initializes option.R_par i.e. PARENT list of sub-directory list
	if (sub_dirs == NULL && sub_next == NULL && option.R_par->next)
	...
	{	
		...
		if (option.R_par->next)
			...
			exit(0);	/* ************ IMPORTANT ******** ends up exiting even though there's one more root sub-dir to recursively analyze */
